...
export async function prepareCodexSandboxNativeContext(params: {
  ...
  const isTrustedOwner = (uid: number) =>
    effectiveUid === undefined || uid === effectiveUid || uid === 0;
  ...
  for (const file of ["config.toml", "hooks.json"]) {
    ...
    if (
      stat &&
      (!stat.isFile() || stat.nlink !== 1 || !isTrustedOwner(stat.uid) || (stat.mode & 0o022) !== 0)
    ) {
      throw new Error(
